function [Bonddis,Anglek,Dihedralk,MASSES,POS]=get_atomistic_data_asphaltCG_MARTINI(prefix,flags)
% prefix='AP';
...
simnums=1;
%% Check Config
Sbead=sort(Beadlist_temp(Beadlist_temp~=0));
Eflag=0;
if min(Sbead)~=1
    Eflag=1;
end
if max(Sbead)~=length(Sbead)
    Eflag=1; 
end
if length(unique(Sbead))~=length(Sbead)
    Eflag=1;
end
if sum(Sbead(2:end)-Sbead(1:end-1))~=length(Sbead)-1
    Eflag=1;
end
if Eflag==1
    error('Bead list does not contain the right number of atoms or it skips atoms')
end
if size(Beadlist_temp,1)~=length(Beadtype)
    error('Number of Martini Beads does not match Beadlist temp')
end
...
if ExtractDataFlag==1
    for i=simnums
%         path=['/mnt/Shared_Data/anic/LONI_output/Small_box/Kspace_pppm_long/Pure_asphaltene_runs/' prefix '/'];
    ...
    for k=1:1
        for i=1:size(Beadlist,2)
            ...
            if ~isempty(Dihedrals)
                ...
                systemwrite.impropers=[];
                systemwrite.impropernames=[];
            end
...
%% Masses
if COMflag==2 || COMflag==1
    system=eval(['system' num2str(1)]);
    for b=1:length(Beadtype)
        atoms1=Beadlist{b};
        MASSES(b)=sum(system.mass(atoms1));
    end
    MASSES
end
...
%% Pos
    ...
    for b=1:length(Beadtype)
        atoms1=Beadlist{b};
        POS(b,:)=COM(system.pos(atoms1,:,1),system.lammpsdim1D(1,:));
    end
    for DD=1:3
        POS(:,DD)=POS(:,DD)-min(POS(:,DD));
    end
