clear all
close all
path_p=['/mnt/Shared_Data/anic/LONI_output/Random_packing/Pure_polymers/Pure_polystyrene/']; %%%Change for polymer
prefix_p=['PS10_poly_'];%%%Change for polymer
path_b=['/mnt/Shared_Data/anic/LONI_output/Random_packing/Pure_asphalt_components/Hopane/']; %%%Change for binder
prefix_b=['Hopane_poly_'];%%%Change for binder
path_mix=['/mnt/Shared_Data/anic/LONI_output/Random_packing/Polystyrene_mix/HP_PS10/']; %%%Change for mix
prefix_mix=['HP_PS10_'];%%%Change for mix
...
[mass_p] = ReadMassFromLAMMPSData([path_p prefix_p '1_replicated.data'])
...
N=5
for i=1:N;
    filename=[path_p prefix_p num2str(i) '.log'];
...
vmpm = (mass_p/512)./(mpm_p.*dmean_p); %volume per mole of monomer for T 400K
...
% mass=66786.304*(8^3)/64+23530.34*(8^3)/27; %PS AAC1
% mass=66786.304*(8^3)/64+27086.022*(8^3)/27; %PS PMMA
[mass_b] = ReadMassFromLAMMPSData([path_b prefix_b '1_replicated.data']) %'_b' denotes binder
%mass=(3*310.60999+843.636) %use for mix with components
mass_b=mass_b*8^3;
...
N=5
for i=1:N;
    filename=[path_b prefix_b num2str(i) '.log'];
    [R] = readlammpslog(filename)
    %tstart=716+250; %use for mix 
    %tstart=716+250-408; % use for pure
    %tstart=700; %for new molecules mix
...
% mass=66786.304*(8^3)/64+23530.34*(8^3)/27; %PS AAC1
% mass=66786.304*(8^3)/64+27086.022*(8^3)/27; %PS PMMA
[mass_mix] = ReadMassFromLAMMPSData([path_mix prefix_mix '1_replicated.data'])
%mass=(3*310.60999+843.636) %use for mix with components
mass_mix=mass_mix*8^3;
...
N=5
for i=1:N;
    filename=[path_mix prefix_mix num2str(i) '.log'];
    [R] = readlammpslog(filename)
    %tstart=716+250; %use for mix 
    %tstart=716+250-408; % use for pure
    %tstart=700; %for new molecules mix
...
U_p_pm = Umean_p./tm_p
U_b_pm = Umean_b./tm_b
mass_mix=(((mass_p/512)*tm_p)./mpm_p.^2)+(((mass_b/512)*tm_b)./mpm_b.^2)
mass_1=((mass_p/512)*tm_p)./mpm_p.^2
mass_2=((mass_b/512)*tm_b)./mpm_b.^2
