...
path_p=['/mnt/Shared_Data/anic/LONI_output/New_files/Kspace_pppm_long/2023/PE_2023/PE30_']; %%%Change for polymer

path_b=['/mnt/Shared_Data/anic/LONI_output/New_files/Kspace_pppm_long/2024/AAM1/AAM1_mix_']; %%%Change for binder

path_mix=['/mnt/Shared_Data/anic/LONI_output/New_files/Kspace_pppm_long/2024/AAM1_PE_25_mix/AAM1_PE_25_mix_']; %%%Change for mix

rep=256;%input('Enter replication number:')
mpm=30;%input('Enter monomer per molecule for polymer:')e
temperature=500;%input('Enter the temperature:')
in=5;%input('Enter the run number')
...
for tstart=timeframe %loops over time
    ...
    vmpm(1)= (mass_p/rep)./(mpm_p.*dcool_p(2));%volume per mole of monomer for T 400K
...
% 
       ...
       %Umean_b=Ucool_b(2)
        ...
        tm_b(1)=rep.*mpm_b(1)                    ;%total number of monomers for binder for T 400K 
        mpm_b(2)=(mass_b/rep)./(vmpm(2).*dcool_b(3));
        tm_b(2)=rep.*mpm_b(2)                    ;%total number of monomers for binder for T 400K 
        mpm_b(3)=(mass_b/rep)./(vmpm(3).*dcool_b(4));
        tm_b(3)=rep.*mpm_b(3)                    ;%total number of monomers for binder for T 400K 
...
%for mix 3
    ...
    mass_mix_m=(((mass_p/rep)*tm_p)./mpm_p.^2)+(((mass_b/rep)*tm_b(2))./mpm_b(3).^2);
    mass_1=((mass_p/rep)*tm_p)./mpm_p.^2;
...
%         set(h1,'XMinorTick','on')
        ...
        chi(3)=-SE_all*tinv(0.975,2)+mean(mean_chi);
        chi(4)=SE_all*tinv(0.975,2)+mean(mean_chi);
