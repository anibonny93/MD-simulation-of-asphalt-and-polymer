clear all
close all
path_p=['/mnt/Shared_Data/anic/LONI_output/Small_box/Kspace_pppm_long/Asphalt_PVC/Asphalt_PVC10/PVC/PVC_']; %%%Change for polymer
%prefix_p=['Polypropylene_'];%%%Change for polymer
path_b=['/mnt/Shared_Data/anic/LONI_output/Small_box/Kspace_pppm_long/Pure_asphaltene_runs/APy/APy_']; %%%Change for binder
%prefix_b=['Asphaltene_phenol_poly_'];%%%Change for binder
path_mix=['/mnt/Shared_Data/anic/LONI_output/Small_box/Kspace_pppm_long/Asphalt_PVC/Asphalt_PVC10/APy_PVC/APy_PVC_']; %%%Change for mix
...
SE_d=dall_p(:,end)/sqrt(N);
dall_p(:,end+1)=-SE_d*tinv(0.975,N-1)+dall_p(:,N+2);
dall_p(:,end+1)=SE_d*tinv(0.975,N-1)+dall_p(:,N+2);
...
SE_d=Uall_p(:,end)/sqrt(N);
Uall_p(:,end+1)=-SE_d*tinv(0.975,N-1)+Uall_p(:,N+2);
Uall_p(:,end+1)=SE_d*tinv(0.975,N-1)+Uall_p(:,N+2);
...
SE_d=dall_b(:,end)/sqrt(N);
dall_b(:,end+1)=-SE_d*tinv(0.975,N-1)+dall_b(:,N+2);
dall_b(:,end+1)=SE_d*tinv(0.975,N-1)+dall_b(:,N+2);
...
SE_d=Uall_b(:,end)/sqrt(N);
Uall_b(:,end+1)=-SE_d*tinv(0.975,N-1)+Uall_b(:,N+2);
Uall_b(:,end+1)=SE_d*tinv(0.975,N-1)+Uall_b(:,N+2);
...
SE_d=dall_mix(:,end)/sqrt(N);
dall_mix(:,end+1)=-SE_d*tinv(0.975,N-1)+dall_mix(:,N+2);
dall_mix(:,end+1)=SE_d*tinv(0.975,N-1)+dall_mix(:,N+2);
...
SE_p=Uall_mix(:,end)/sqrt(N);
Uall_mix(:,end+1)=-SE_d*tinv(0.975,N-1)+Uall_mix(:,N+2);
Uall_mix(:,end+1)=SE_d*tinv(0.975,N-1)+Uall_mix(:,N+2);
