...
k=convhull(phi,z)
[xk,zk]=deal(phi(k),z(k));
j = diff(xk)>0;
[xk,zk]=deal(xk(j),zk(j));
interDist=vecnorm(  diff([xk(:),zk(:)],1,1) ,2,2);
[~,imax]=max(interDist);
x1=xk(imax); y1=zk(imax);      %the two tangent points (x1,y1) and (x2,y2)
x2=xk(imax+1); y2=zk(imax+1);
close all
hold on
plot(phi,polyval(phi,z))
